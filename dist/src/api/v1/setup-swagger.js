...
**Creation / Edition / Replacement / Suppression:**
  ...
  - \`<METHOD> /user/<user_id>/<resources>/<resource_id>\`
    ...
    if (process.env.API_VERSION) {
        ...
        documentBuilder.setVersion(process.env.API_VERSION).addServer(ROOT_URL);
    }
    const SWAGGER_ROOT_URL = `${process.env.SWAGGER_ROOT_URL}/documentation`;
    const document = swagger_1.SwaggerModule.createDocument(app, documentBuilder.build());
    swagger_1.SwaggerModule.setup(SWAGGER_ROOT_URL, app, document, {
        swaggerOptions: {
            persistAuthorization: true,
        },
    });
    console.info(`Documentation: http://localhost:${process.env.PORT}${SWAGGER_ROOT_URL}`);
}
